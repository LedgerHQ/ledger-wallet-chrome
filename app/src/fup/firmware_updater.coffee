  ...
  getFirmwareUpdateAvailability: (wallet, bootloaderMode = no, forceBl = no, callback = undefined) ->
    ...
    wallet.getRawFirmwareVersion bootloaderMode, forceBl, (version, error) =>
      ...
      if ledger.fup.compareVersions(@_lastVersion, ledger.fup.versions.Nano.CurrentVersion.Os).eq()
        completion.success(result: FirmwareAvailabilityResult.Same, available: yes, dongleVersion: version, currentVersion: ledger.fup.versions.Nano.CurrentVersion.Os)
