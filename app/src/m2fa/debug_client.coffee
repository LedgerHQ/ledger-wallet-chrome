...
class @ledger.m2fa.DebugClient extends ledger.m2fa.Client
  constructor: (pairingId, baseUrl) ->
    ...
    @on 'm2fa.challenge.sended', (e,challenge) => console.log("%c[M2FA][#{@pairingId}] challenge :", "color: #888888", challenge, e)
  ...
  challenge: (data) -> @_onChallenge({data: data})
  accept: -> @_onAccept()
  repeat: -> @_onRepeat()
  ...
  _computeChallenge: (challenge) ->
    ...
    [cardChallenge, pairingKey] = [bytes[0...4], bytes[4..-1]]
  ...
  _generateSessionKey: ->
    ...
    l((Convert.toHexByte(i) for i in secret).join(''))
