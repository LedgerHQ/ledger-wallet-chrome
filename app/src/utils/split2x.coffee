...
class ledger.split2x
  ...
  @checkUtxoOn2x: () ->
    ...
    cache = new ledger.wallet.Wallet.Cache('split_cache', hdWallet)
    cache.initialize ->
      ...
      _recoverBatch: (batch, accountIndex, syncToken) ->
        ...
        _recoverAddresses(account.getRootDerivationPath(), from, to, blockHash, syncToken).then (result) =>
          ...
          addresses = hdWallet.getAllAddresses()
          addresses_list = []
          for list in addresses
            for address in list
              addresses_list.push(address.value)
          for tx in result['txs']
            for ouput in outputs
              if addresses_list.includes(output.address)
                if !_txo[tx.hash]?
                  _txo[tx.hash] = {}
            ...
            for input in inputs
              if addresses_list.includes(input.address)
                if !_blocks[tx.block.height]?
                  _blocks[tx.block.height] = {block: tx.block, txo: [], txi: {}}
                if !_blocks[tx.block.height].txi[tx.hash]?
                  _blocks[tx.block.height].txi[tx.hash] = []
                _blocks[tx.block.height].txi[tx.hash].push(input.output_index)
